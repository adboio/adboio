...
export const getBuildLogEntry = cache(async (id: string): Promise<BuildLogEntry | null> => {
  const supabase = createClient();
...
export async function createBuildLogEntry(
  ...
  return {
    ...
    build_links: socialLinks?.map((link) => ({ platform: link.platform, link: link.url })) || [],
  };
}
...
export async function updateBuildLogEntry(
  ...
  if (images !== undefined) {
    ...
    const pathsToDelete = existingPaths.filter(path => !newPaths.includes(path));
  ...
  if (socialLinks !== undefined) {
    ...
    const existingLinkStrings = existingLinks?.map(l => `${l.platform}:${l.link}`) || [];
    ...
    // Find links to add
    const linksToAdd = socialLinks.filter(newLink =>
